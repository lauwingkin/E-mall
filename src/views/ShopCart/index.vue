...
export default {
  ...
  methods: {
    getData() {
      this.$store.dispatch("getCartList"); //获取个人购物车数据
    },
    handler: throttle(async function (type, disNum, cart) {
      //添加节流功能
      //type 修改行为
      //disNum新数量,cart取得原数量及商品id
      switch (type) {
        case "add":
          disNum = 1;
          break;
    ...
    async updateChecked(cart, event) {
      try {
        ...
        let checked = event.target.checked ? "1" : "0"; //布尔值转数字
    ...
    async updateAllCartChecked(event){
      ...
      console.log("updateAllCartIsChecked")
  ...
  computed: {
    ...
    ...mapGetters(["cartList"]),
    cartInfoList() {
      return this.cartList.cartInfoList || []; //防undefined
    },
    //计算总价
    totalPrice() {
      let sum = 0;
