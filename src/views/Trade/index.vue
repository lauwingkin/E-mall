...
export default {
  ...
  methods: {
    ...
    async submitOrder() {
      ...
      let data = {
        consignee: this.userDefaultAddress.consignee,   //匹配数据
      ...
      //传参
      try{
        let result =await this.$API.reqSubmitOrder(tradeNo,data);  //触发api,并传参
        console.log(result);
        if(result.code==200){
          this.orderId = result.data;  //把返回的订单id保存
          //路由跳转+传参
